...
sensors3 = { "temperatureHumiditySensor.temperature": 999,
             "temperatureHumiditySensor.humidity": 999 }
...
#      grovepi.pinMode(self.port, "OUTPUT")
   ...
   def getValue3(self):
      print "getting temperature and humidity values v3..."
      temp = 0.0
      humidity = 0.0
      # The first parameter is the port, the second parameter is the type of sensor.
      [temp,humidity] = grovepi.dht(self.port,blue)
      if math.isnan(temp) == False and math.isnan(humidity) == False:
          print "temp = %.02f C humidity =%.02f%%"%(temp, humidity)
          temp_temperature = int(temp)
          temp_humidity    = int(humidity)
      else:
          temp_temperature = 999
          temp_humidity    = 999


      sensors3["temperatureHumiditySensor.temperature"] = temp_temperature
      sensors3["temperatureHumiditySensor.humidity"]    = temp_humidity
